...
public final class NonEmptyParagraph<PS, SEG, S> implements Paragraph<PS, SEG, S> {
    ...
    @SafeVarargs
    public NonEmptyParagraph(PS paragraphStyle, SegmentOps<SEG, S> segmentOps, SEG text, SEG... texts) {
        this(paragraphStyle, segmentOps, list(text, texts));
    }

    NonEmptyParagraph(PS paragraphStyle, SegmentOps<SEG, S> segmentOps, List<SEG> segments) {
    ...
    public Paragraph<PS, SEG, S> concat(Paragraph<PS, SEG, S> p) {
        ...
        SEG right = p.getSegments().get(0);
        Optional<SEG> joined = segmentOps.join(left, right);
        if(joined.isPresent()) {
            SEG segment = joined.get();
            List<SEG> segs = new ArrayList<>(segments.size() + p.getSegments().size() - 1);
            segs.addAll(segments.subList(0, segments.size()-1));
            segs.add(segment);
            segs.addAll(p.getSegments().subList(1, p.getSegments().size()));
            return new NonEmptyParagraph<>(paragraphStyle, segmentOps, segs);
        } else {
            List<SEG> segs = new ArrayList<>(segments.size() + p.getSegments().size());
    ...
    public Paragraph<PS, SEG, S> trim(int length) {
        if(length >= length()) {
            ...
            return segs.isEmpty()
                    ...
                    : new NonEmptyParagraph<>(paragraphStyle, segmentOps, segs);
        }
    }
    ...
    public Paragraph<PS, SEG, S> restyle(int from, StyleSpans<? extends S> styleSpans) {
        ...
        for(StyleSpan<? extends S> span: styleSpans) {
            ...
            middleSegs.addAll(text.restyle(span.getStyle()).getSegments());
            offset = end;
        }
        Paragraph<PS, SEG, S> newMiddle = new NonEmptyParagraph<>(paragraphStyle, segmentOps, middleSegs);
    ...
    public boolean equals(Object other) {
        if(other instanceof NonEmptyParagraph) {
            NonEmptyParagraph<?, ?, ?> that = (NonEmptyParagraph<?, ?, ?>) other;
            return Objects.equals(this.paragraphStyle, that.paragraphStyle)
                && Objects.equals(this.getSegments(), that.getSegments());
        } else {
            return false;
        }
    }
